...
def input_file_obj():
    obj = omcore.File(
        ...
        storage_size=omcore.QuantitativeValue(value=34.0, unit=omterms.UnitOfMeasurement.byte)
    )
    obj.save(kg_client, space=TEST_SPACE, recursive=True)
    yield obj
    obj.delete(kg_client)
...
@pytest.fixture(scope="module")
def output_file_objs():
    objs = [
        omcore.File(
            id=f"{ID_PREFIX}/{uuid4()}",
            content_description="File generated by some computation",
            format=omcore.ContentType.image_png,
            ...
            storage_size=omcore.QuantitativeValue(value=60715.0, unit=omterms.UnitOfMeasurement.byte)
        ),
        omcore.File(
            id=f"{ID_PREFIX}/{uuid4()}",
            content_description="File generated by a simulation",
            format=omcore.ContentType.application_vnd_nwb_nwbnplushdf,
            ...
            storage_size=omcore.QuantitativeValue(value=605888.0, unit=omterms.UnitOfMeasurement.byte)
        ),
    ]
    for obj in objs:
        obj.save(kg_client, space=TEST_SPACE, recursive=True)
...
@pytest.fixture(scope="module")
def hardware_obj():
    obj = omcmp.HardwareSystem.by_name("CSCS Castor", kg_client, release_status="any", space="common")
...
def data_analysis_obj(person_obj, input_file_obj, output_file_objs, software_version_objs,
                      environment_obj, launch_config_obj):
    timestamp = datetime.now()
    resource_usage = [omcore.QuantitativeValue(value=2.0,
                                               unit=omterms.UnitOfMeasurement.hour)]
    obj = omcmp.DataAnalysis(
        ...
        status=omterms.ActionStatusType.potential,  # i.e. queued
...
def visualisation_obj(person_obj, input_file_obj, output_file_objs, software_version_objs,
    ...
    resource_usage = [omcore.QuantitativeValue(value=29.0,
                                               unit=omterms.UnitOfMeasurement.second)]
    obj = omcmp.Visualization(
        ...
        started_by=person_obj,
        status=omterms.ActionStatusType.potential,
        resource_usages=resource_usage,
        tags=["string"]
    )
    obj.save(kg_client, space=TEST_SPACE, recursive=True)
...
class TestGetDataAnalysis:
    ...
    def test_get_data_analysis(self, data_analysis_obj, environment_obj, software_version_objs):
        ...
        expected = {
            "description": None,
            "end_time": "2021-05-28T18:32:58.597000Z",
...
class TestGetVisualisation:
    ...
    def test_get_visualisation(self, visualisation_obj, environment_obj, software_version_objs):
        ...
        expected = {
            ...
            "start_time": "2021-06-28T16:32:58Z",
            "started_by": {"family_name": "Baggins",
                        "given_name": "Bilbo",
                        "orcid": "http://orcid.org/0000-0002-4793-7541"},
            "status": "queued",
...
class TestCreateWorkflowRecipe:
    ...
    def test_post_recipe(self, person_obj):
        existing_recipe = omcmp.WorkflowRecipeVersion.list(kg_client, short_name="PSD_workflow_KG", version_identifier="12345678", release_status="in progress")
        if existing_recipe:
